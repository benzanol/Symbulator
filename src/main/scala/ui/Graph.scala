...
object Graph {
	...
	// Draw the graph
	var expressions = List[Sym]()

	var functions = Seq[Double => Double]()

	@JSExportTopLevel("updateFunctions")
	def updateFunctions(): Unit = {
		val output = js.eval("getLatexEquations()")
		val strings = output.asInstanceOf[js.Array[String]].toSeq
